...
public class MainClass {
	...
	public MainClass() {
		try {
			...
			// System.exit(1); // Potentially problematic exit, consider throwing an exception instead
		}
	}

	/**
	 * The main method serves as the entry point of the application.
     * It initializes necessary components and fetches configuration parameters from a JSON file specified as a command-line argument.
     * Expected Input: 
     * - String array of command-line arguments (args)
     * Expected Output: 
     * - None directly, but the application initializes based on config.
     */
	public static void main(String[] args) {
		try {
			...
			// Validate command-line arguments: Expecting exactly one argument (configuration file)
			if (argc != 1) { // Error handling for incorrect number of parameters
				System.out.println("Usage(" + argc + "): <command> <configuration file.json>"); // Indicate expected usage
				...
				try {
					...
					XmlRpcQmx.setServerPort(jsonObj.has("ServerPort") ? jsonObj.getString("ServerPort") : XmlRpcQmx.getServerPort());
	...
	/**
	 * Converts a float value to a scaled string representation in hexadecimal format.
     * Expected Input:
     * - float value: the value to be scaled
     * - float valuemin: the minimum value of the range
     * - float valuemax: the maximum value of the range
     * - int range: the scaling range to apply
     * Expected Output:
     * - A string representing the scaled hexadecimal value.
     */
	public static String intToStringScaled(float value, float valuemin, float valuemax, int range) {
		...
		dispDebug("range convert val:" + value + " min:" + valuemin + " max:" + valuemax + " range:" + range + " --> " + retval); // Debug output
     ...
     * Expected Input:
     * - String arg: the string to be converted
     * Expected Output:
     * - int: the integer value derived from the string, defaults to 0 on error.
     */
	...
	/**
	 * Converts a string to a float after stripping non-numeric characters.
     * Handles potential conversion errors gracefully.
     * Expected Input:
     * - String arg: the string to be converted
     * Expected Output:
     * - float: the float value derived from the string, defaults to 0 on error.
     */
	...
	/**
	 * Fetches the corresponding mode string for a given integer identifier (mapped from UICodeMap).
     * Expected Input:
     * - int ival: the integer identifier for the mode
     * Expected Output:
     * - String: the mode string associated with the identifier, or null if not found.
     */
	...
	/**
	 * Retrieves the step size in Hertz based on a string identifier.
     * Expected Input:
     * - String stepStr: the string identifier for the step size
     * Expected Output:
     * - int: the corresponding step size in Hertz, or 0 if not found.
     */
     ...
     * Expected Input:
     * - String sval: the string identifier for the mode
     * Expected Output:
     * - int: the integer representation of the mode, determined by the step string.
     */
	...
	/**
	 * Utility method to display debug messages when debug mode is enabled.
     * Expected Input:
     * - String $msg: the message to be displayed
     * Expected Output:
     * - None. Outputs to console if debug is enabled.
     */
     ...
     * Expected Input:
     * - String json_file: the path to the JSON file
     * Expected Output:
     * - JSONObject: the parsed JSON object, or exits on IOException.
     */
	...
	/**
	 * Retrieves the current callsign.
     * Expected Output:
     * - String: the current callsign held in myCall.
     */
	...
	/**
	 * Sets the callsign to a new value.
     * Expected Input:
     * - String myCallin: the new callsign to set
     * Expected Output:
     * - None. Updates the myCall variable.
     */
	...
	/**
	 * Converts an integer frequency to a human-readable string in MHz.
     * Expected Input:
     * - int ifreq: the frequency in Hz
     * Expected Output:
     * - String: the frequency formatted as a string in MHz.
     */
