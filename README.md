...
~~The code has been modified to send frequency data to SDR++ program via its rigctld emulation server; should an SDR++ instance not be running ( or not have the rigctld running it quietly ignores it)    This therefore adds a band-scope to the QMX.~~  This now includes a band-scope and waterfall built in - this is currently underdevelopment.  If this is enabled it sends commands to the QMX to ensure audio card is in I/Q mode (will build in).
